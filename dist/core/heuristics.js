...
class MovenHeuristicsEngine {
    static evaluate(state) {
        ...
        if (opts.enablePromptInjectionFirewall !== false) {
            ...
            if (!state._userPromptScanned) {
                ...
                if (userInput) {
                    const promptInspection = prompt_firewall_1.MovenPromptInjectionFirewall.inspect(userInput, firewallConfig);
            ...
            if (state.toolCalls.length > 0) {
                const lastCall = state.toolCalls[state.toolCalls.length - 1];
                const inspection = prompt_firewall_1.MovenPromptInjectionFirewall.inspect({ args: lastCall.args, reasoning: lastCall.reasoning }, firewallConfig);
        ...
        if (opts.semanticFingerprint?.enabled !== false && state.reasoningSteps.length >= 3) {
            const sfResult = semantic_fingerprint_1.SemanticFingerprintEngine.evaluate(state.reasoningSteps, state.intentHashes, opts.semanticFingerprint);
            if (sfResult.tripped) {
                const lastCall = state.toolCalls[state.toolCalls.length - 1];
     ...
     *   }
     */
    static async evaluateAsync(state) {
        // Run all synchronous checks first
        const syncResult = this.evaluate(state);
        if (syncResult.tripped)
            return syncResult;
        const opts = state.options;
        if (opts.enableLlmJudgeArbitrator === false)
            return { tripped: false };
     ...
     */
    static async runAsyncJudge(state) {
        const selectedModel = state.getCheaperModel(state.options.provider || state.options.judgeModel);
        ...
        const allReasoningSimilar = recentSteps.length >= 2
            && semantic_fingerprint_1.SemanticFingerprintEngine.stepSimilarity(recentSteps[0], recentSteps[recentSteps.length - 1]) > 0.88;
