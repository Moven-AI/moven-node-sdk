 ...
 */
function failSafe(detectorName, fn, fallback) {
    try {
        return fn();
    }
    catch (err) {
        logger_1.MovenLogger.error(`Circuit breaker sub-detector '${detectorName}' threw an internal error and was skipped (fail-open). Hard ceilings remain active.`, { detector: detectorName, error: err?.message || String(err) });
        return fallback;
    }
}
class MovenHeuristicsEngine {
    static evaluate(state) {
        ...
        const burnGuardResult = failSafe('burn_guard', () => burn_guard_1.MovenOvernightBurnGuard.evaluate(state), { tripped: false });
        ...
        const hallucinationResult = failSafe('hallucination', () => hallucination_1.MovenHallucinationDetector.evaluate(state), { tripped: false });
        ...
        if (opts.enablePromptInjectionFirewall !== false) {
            ...
            if (!state._userPromptScanned) {
                ...
                if (userInput) {
                    const promptInspection = failSafe('firewall_user_prompt', () => prompt_firewall_1.MovenPromptInjectionFirewall.inspect(userInput, firewallConfig), { isAttack: false, confidence: 0 });
            ...
            if (state.toolCalls.length > 0) {
                const lastCall = state.toolCalls[state.toolCalls.length - 1];
                const inspection = failSafe('firewall_tool_args', () => prompt_firewall_1.MovenPromptInjectionFirewall.inspect({ args: lastCall.args, reasoning: lastCall.reasoning }, firewallConfig), { isAttack: false, confidence: 0 });
        ...
        if (state.cumulativeCost >= (opts.maxCostDollar || 2.00)) {
            ...
            if (opts.softCostCeiling === true) {
                ...
                if (isMakingProgress)
                    effectiveCap = effectiveCap * 1.25;
            }
        ...
        if (userDirected && lastToolCall) {
            ...
            for (let i = state.toolCalls.length - 1; i >= 0; i--) {
                ...
                if (!c.humanAttested)
                    break;
                if (c.resultHash && refHash && c.resultHash === refHash)
                    stagnantAttested += 1;
                else
                    break;
            }
        ...
        if (!inGrace && !userDirected) {
            ...
            if (opts.enableSemanticFingerprint !== false &&
                ...
                const sfResult = failSafe('semantic_fingerprint', () => semantic_fingerprint_1.SemanticFingerprintEngine.evaluate(state.reasoningSteps, state.intentHashes, opts.semanticFingerprint, { skipGoalStateHash }), { tripped: false });
                if (sfResult.tripped) {
                    const lastCall = state.toolCalls[state.toolCalls.length - 1];
            ...
            if (opts.layer2?.enabled !== false && state.toolCalls.length > 0 && state.layer2Guard) {
                const lastCall = state.toolCalls[state.toolCalls.length - 1];
                const l2Result = failSafe('layer2_semantic_guard', () => state.layer2Guard.evaluate(lastCall.toolName, lastCall.args, state.userRequest), { decision: 'ALLOW', reason: 'Layer 2 guard internal error — fail-open.' });
            ...
            if (state.toolCalls.length > 0) {
                const adaptiveResult = failSafe('adaptive_loop', () => adaptive_loop_1.MovenAdaptiveLoopEngine.evaluate(state.toolCalls, opts.maxRepeatCalls ? Math.max(opts.maxRepeatCalls * 3, 15) : 15), { tripped: false, category: 'COMPUTATIONAL', noveltyScore: 1, entropyGain: 1, consecutiveCount: 0, isLegitimateExploration: true });
        ...
        if (opts.customCheck) {
            const customRes = failSafe('custom_check', () => opts.customCheck(state), null);
...
exports.MovenHeuristicsEngine = MovenHeuristicsEngine;
