...
class MovenKillHandler {
     ...
     */
    static FALLBACK_GRACE_STEPS = 3;
    static emitDecisionSpan(decision, tripResult, state) {
        (0, otel_1.recordDecisionSpan)({
            decision,
    ...
    static async handleTripResult(tripResult, state, reporter) {
        ...
        if (state.options.dryRun) {
            logger_1.MovenLogger.warn(`[DRY RUN MODE] Circuit breaker simulated trip for heuristic '${tripResult.heuristic || 'repeat_tool_call'}': ${tripResult.reason}. Execution continues without interruption.`, { runId: state.runId, dryRun: true });
            if (reporter) {
        ...
        if (state.options.pauseOnTrip) {
            ...
            logger_1.MovenLogger.warn(`[PAUSE & ASK] Agent '${state.agentName}' paused on suspicious activity for developer verification. Webhook dispatched.`, { runId: state.runId, resumeToken });
            ...
            throw new errors_1.MovenPauseError({
                runId: state.runId,
        ...
        if (state.options.autoFallbackCheaperModel && !state.isFallbackActive && !state.halted && tripResult.heuristic !== 'prompt_injection') {
            ...
            logger_1.MovenLogger.warn(`[Auto-Fallback Activated] Routing agent '${state.agentName}' to cheaper model '${cheaperModel}' instead of terminating run.`, { runId: state.runId, cheaperModel });
    ...
    static async executeKill(tripResult, state, reporter) {
        ...
        if (state.markKillInitiated()) {
            ...
            logger_1.MovenLogger.debug('Duplicate kill suppressed (single-flight guard).', { runId: state.runId, toolName: tripResult.toolName });
        }
    ...
    static runKillSideEffects(tripResult, state, reporter) {
        const error = new errors_1.MovenKillError({
            runId: state.runId,
            heuristic: tripResult.heuristic || 'repeat_tool_call',
            reason: tripResult.reason || 'Circuit breaker tripped',
        ...
        if (logger_1.MovenLogger.isEnabled('error')) {
            ...
            if (tripResult.toolName) {
                ...
                console.error(`\x1b[31m\x1b[1m📋 ARGUMENTS:\x1b[0m \x1b[90m${(0, safe_json_1.safeStringify)(tripResult.toolArgs || {})}\x1b[0m`);
            }
        }
        ...
        logger_1.MovenLogger.error(`CIRCUIT BREAKER TRIPPED — agent '${state.agentName}' killed. Money saved: $${moneySaved >= 0.01 ? moneySaved.toFixed(2) : moneySaved.toFixed(4)}`, {
            runId: state.runId,
            agentName: state.agentName,
            heuristic: tripResult.heuristic || 'repeat_tool_call',
            reason: tripResult.reason,
        ...
        if (state.options.onHallucination) {
            try {
                ...
                logger_1.MovenLogger.error('Error inside onHallucination callback:', { error: err?.message || String(err) });
            }
        }
        if (state.options.onKill) {
            try {
                ...
                logger_1.MovenLogger.error('Error inside onKill callback:', { error: err?.message || String(err) });
            }
        }
        // Report event asynchronously to hosted backend
        if (reporter) {
            reporter.reportKillEvent(error, state).catch(err => {
                logger_1.MovenLogger.warn('Failed to report kill event:', { error: err?.message || String(err) });
            });
        }
    }
